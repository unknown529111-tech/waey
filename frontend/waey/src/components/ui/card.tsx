...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(
      "rounded-3xl border border-border/50 bg-card text-card-foreground shadow-soft hover:-translate-y-1 hover:shadow-moss-lg transition-all duration-300",
      className,
    )}
    {...props}
  />
...
const CardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("text-sm text-muted-foreground leading-[1.9]", className)} {...props} />
  ),
);
