  ...
  return (
    <div className="bg-card rounded-3xl p-5 border border-border">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-bold text-sm">ختام اليوم</h3>
      </div>
      <div className="space-y-3">
        <div>
          <label className="text-xs font-bold flex items-center gap-1.5 mb-1.5">
            <Trophy className="size-3.5 text-accent" /> إنجاز اليوم
          </label>
          <input value={achievement} onChange={(e) => setAchievement(e.target.value)} placeholder="أحسن حاجة عملتها النهارده..." className="w-full bg-muted/50 border border-border rounded-2xl p-2.5 text-xs outline-none focus:border-primary/40 transition-colors" />
        </div>
        <div>
          <label className="text-xs font-bold flex items-center gap-1.5 mb-1.5">
            <Lightbulb className="size-3.5 text-yellow-500" /> درس بكره
          </label>
          <input value={lesson} onChange={(e) => setLesson(e.target.value)} placeholder="حاجة هظبطها بكرة..." className="w-full bg-muted/50 border border-border rounded-2xl p-2.5 text-xs outline-none focus:border-primary/40 transition-colors" />
        </div>
        <button onClick={() => setScreensOff(!screensOff)} className={`flex items-center gap-2 w-full p-3 rounded-2xl text-xs font-bold transition-all ${screensOff ? "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600" : "bg-secondary text-muted-foreground"}`}>
          ...
          {screensOff ? "✓ قفلت الشاشات — استعد للنوم" : "اضغط لو قفلت الشاشات هبدأ روتين النوم"}
        </button>
      </div>
    </div>
  );
